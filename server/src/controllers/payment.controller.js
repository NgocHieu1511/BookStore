...
class PaymentController {
  async createPayment(req, res) {
    //lấy ra trạng thái thanh toán từ body
    const { typePayment } = req.body;
    //lấy ra id của user từ token
    const id = req.user;
    ...
    // THANH TOÁN VNPAY
      ...
      const vnpay = new VNPay({
        tmnCode: "J0SY0QKY",
        secureSecret: "TMAORHXOUDZRJGMDFQGGPRTDSMPDSXZC",
  ...
  //lấy ra dữ liệu từ vnpay trả về
  ...
  //CHỨC NĂNG LẤY DANH SÁCH ĐƠN HÀNG CỦA ADMIN
  ...
  async updatePayment(req, res) {
    // lấy ra orderId và status do nguoi dùng gửi lên
    const { orderId } = req.params;
    const { status } = req.body;
    //kiểm tra nếu không có thì trả về lỗi
    if (!orderId || !status) {
