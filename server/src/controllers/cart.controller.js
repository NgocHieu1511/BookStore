...
class CartController {
  ...
  //CHỨC NĂNG CẬP NHẬT MÃ GIẢM GIÁ CHO GIỎ HÀNG
  async applyCoupon(req, res) {
    // lấy ra id của nguoi dùng
    const id = req.user;
    // lấy ra mã giảm giá mà người dùng gửi từ body
    ...
    switch (true) {
      ...
      case findCoupon.minPrice > findCartUser.totalPrice:
        throw new BadRequestError(
          `Mã giảm giá chỉ áp dụng cho đơn hàng từ ${findCoupon.minPrice}`,
        );
      default:
        break;
    }
    // nếu mã giảm giá hợp lệ thì áp dụng cho giỏ hàng
    //lấy ra thông tin giỏ hàng của người dùng
    findCartUser.couponId = findCoupon._id;
    //tính lại giá cuối cùng của giỏ hàng sau khi áp dụng mã giảm giá
    findCartUser.finalPrice =
      findCartUser.totalPrice -
      (findCartUser.totalPrice * findCoupon.discount) / 100;
    //lưu lại thông tin giỏ hàng sau khi áp dụng mã giảm giá
    await findCartUser.save();
    return new OK({
